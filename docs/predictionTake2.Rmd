...
nn1 <- caret::train(f,
             ...
             method = "mlp",
             tuneGrid = expand.grid(size = c(2, 3, 4, 5, 10)),
...
# 
# pred <- predict(nn1, training$transformed)
# comp <- cbind(training$transformed, prediction = pred) %>%
#   mutate(correct = survived == prediction)
```
```{r}
nn1$bestTune$size
...
```{r}
nn_2_lc <- nn_learning_curves(training$transformed %>% select(-passengerid), validation$transformed %>% select(-passengerid), 25, 2)
write.csv(nn_2_lc, file = 'output/nn_2_lc.csv')
#nn_2_lc <- read_csv('output/nn_2_lc.csv')
plot_learning_curves(nn_2_lc, "Neural Network with 2 hidden nodes.")
```



The learning curves suggest that the model with 2 hidden nodes may be overfitting the data, but we can't make a simpler neural network, as a network with only 1 hidden node will be equivalent to logistic regression.

We will use this model as our Neural Network candidate model:
```{r}
...
nn_pred <- predict(nn_model, test_data %>% transform_variables())
sub <- test_data %>% 
  mutate(Survived = nn_pred) %>% 
  select(passengerid, Survived) %>% 
  rename(PassengerId = passengerid)
...
Neural Network         `r round(nn_accuracy(nn_model,validation$transformed)$accuracy * 100, 2)`     76.08
