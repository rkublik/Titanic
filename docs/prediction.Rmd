...
nn_dummy <- dummyVars(~ ., data = nn_train)
nntrain <- data.frame(predict(nn_dummy, newdata = nn_train)) %>% 
...
nntest <- data.frame(predict(nn_dummy, newdata = nn_test)) %>% 
  mutate(survived = (survived.yes)) %>% 
  select(-survived.yes, -survived.no)
nntrain %>% names()
nntest %>% names()
...
nn <- neuralnet(f, data = nntrain, hidden = 5, linear.output = FALSE)

plot(nn)

nn_pred = compute(nn, nntrain %>% select(-survived))
nn_pred$net.result %>% round() %>% head()
```
