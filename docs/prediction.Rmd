...
```{r}
results3 <- evaluate_model(data$train %>% select(survived, pclass, parch, embarked, sex, age, familysize),
...
```{r}
results4 <- evaluate_model(data$train %>% select(survived, pclass, parch, embarked, sex, age, familysize),
                           data$test %>% select(survived, pclass, parch, embarked, sex, age, familysize), interactions = TRUE)
imp <- varImp(results4$model, scale = FALSE)
plot(imp)
```
Adding in the ticket, gives 71% accuracy.
```{r include = FALSE}
results5 <- evaluate_model(data$train %>% select(survived, pclass, parch, embarked, sex, age, familysize,ticket),
                           data$test %>% select(survived, pclass, parch, embarked, sex, age, familysize, ticket), interactions = FALSE)
imp <- varImp(results5$model, scale = FALSE)
plot(imp)
```
nope...
