...
model_accuracy <- function(train_data, cv_data){
  ...
  train_error <- 1 - calc_accuracy(train_pred, train_data$survived)
  
  cv_pred <- predict(model, cv_data, type = "prob")
  cv_error <- 1 - calc_accuracy(cv_pred, cv_data$survived)
  
  list("train" = train_error, "cv" = cv_error)
}
...
plot_learning_curves <- function(plotdf, model){
    ggplot(data = plotdf, aes(x = nrows)) +
      ...
      geom_line(aes(y = cv_error, color = "Validation"), size = 1) +
...
nn_error <- function(model, data){
  ...
  list("com" = comp, "error" = 1 - accuracy$accuracy[1])
}

nn_learning_curves <-  function(train, test, step, nhidden){
  nrows <- seq(from = 2, to = nrow(train), by = step)
    #c(2:nrow(train))
  train_error <- rep(0, length(nrows))
  cv_error <- rep(0, length(nrows))
  ...
  for (i in 1:length(nrows)) {
    ...
    train_error[i] <- nn_error(nn, train %>% head(nrows[i]))$error
    cv_error[i] <- nn_error(nn, test)$error
  }
