...
```
We begin by including all the variables we determined to be very or somewhat important.
```{r}
#test function:
results1 <- evaluate_model(data$train %>% select(survived, pclass, sex, sibsp, parch, embarked, title),
                           data$test %>% select(survived, pclass, sex, sibsp, parch, embarked, title))
print(results1$conf$table)
print(results1$perf)
```
...
model_accuracy <- function(train_data, cv_data){
  model <- train(survived ~ pclass + sex + sibsp + parch + embarked + title,
                  data = train_data,
                  method = "glm",
                  metric = "ROC",
                  trControl = train_control,
...
for (i in 1:length(nrows)) {
  adata <-  model_accuracy(data$train %>% head(nrows[i]), data$test)
  train_accuracy[i] <- adata$train
  cv_accuracy[i] <- adata$cv
}

plotdf <- data.frame(cbind(nrows, train_accuracy, cv_accuracy))
ggplot(data = plotdf, aes(x = nrows, y = train_accuracy)) +
  geom_line() + 
  geom_line(aes(y = cv_accuracy))
...
```{r}
glm.comp <- data$test %>% 
  cbind(prediction = glm.pred$survived)
#False positives:
